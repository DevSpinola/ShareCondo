...
const AnunciosPage = () => {
  ...
  const handleAddAnuncio = async (dadosNovoAnuncio) => {
    if (!usuarioLogado) {
      setFeedbackMessage({
        type: "error",
        text: "Você precisa estar logado para criar um anúncio.",
      });
      return;
    }
    ...
    try {
      ...
      setFeedbackMessage({
        type: "success",
        text: "Anúncio adicionado com sucesso!",
      });
      ...
      const errorMsg =
        ...
        err.response?.data ||
        "Erro ao adicionar anúncio. Tente novamente.";
  ...
  const handleSendOffer = async (offerDataFrontend) => {
    ...
    if (!currentAnuncioForOffer || !currentAnuncioForOffer.id) {
      setFeedbackMessage({
        type: "error",
        text: "Anúncio alvo não selecionado.",
      });
      return;
    }
    ...
    const payload = {
      ...
      valor:
        offerDataFrontend.tipoOferta.toUpperCase() === "DINHEIRO"
          ? parseFloat(offerDataFrontend.valor)
          : null,
      descricao:
        offerDataFrontend.tipoOferta.toUpperCase() === "ITEM" ||
        offerDataFrontend.tipoOferta.toUpperCase() === "SERVICO"
          ? offerDataFrontend.descricao
          : null,
    };
    ...
    try {
      ...
      setFeedbackMessage({
        type: "success",
        text: "Oferta enviada com sucesso!",
      });
  ...
  return (
    <div className="anuncios-page-container">
      ...
      {feedbackMessage.text && (
        <p className={`feedback-message ${feedbackMessage.type}`}> {/* Alterado aqui */}
          {feedbackMessage.text}
        </p>
      )}
